      ...
      <div className="login-page">
        ...
        <div className="login-form-panel">
          <div className="login-card">
            {!token ? (
              <>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: '#dc2626', margin: '0 0 12px' }}>Invalid link</h2>
                <p style={{ color: '#6b7280', fontSize: 14 }}>This reset link is missing or malformed.</p>
                <Link to="/forgot-password" style={{ display: 'block', marginTop: 16, color: '#2563eb', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
              ...
              <>
                ...
                <h2 style={{ fontSize: 22, fontWeight: 700, color: '#1a1f2e', textAlign: 'center', margin: '0 0 8px' }}>Password updated!</h2>
                <p style={{ fontSize: 14, color: '#6b7280', textAlign: 'center' }}>
                  Redirecting you to sign in…
                </p>
                <Link to="/login" style={{ display: 'block', marginTop: 20, textAlign: 'center', color: '#2563eb', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
                  Sign in now
                </Link>
              </>
            ) : (
              <>
                <h2 style={{ fontSize: 22, fontWeight: 700, color: '#1a1f2e', margin: '0 0 6px' }}>Set new password</h2>
                <p style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>Choose a strong password — at least 6 characters.</p>

                {error && <div className="login-error">{error}</div>}

                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#444', marginBottom: 5 }}>New password</label>
                    <input type="password" className="login-input" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="At least 6 characters" autoFocus required />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#444', marginBottom: 5 }}>Confirm new password</label>
                    <input type="password" className="login-input" value={confirmPassword} onChange={e => setConfirm(e.target.value)} placeholder="Repeat password" required />
                  </div>
                ...
                <div style={{ marginTop: 20, textAlign: 'center' }}>
                  <Link to="/login" style={{ color: '#6b7280', fontSize: 13, textDecoration: 'none' }}>← Back to sign in</Link>
                </div>
              </>
            )}
          </div>
