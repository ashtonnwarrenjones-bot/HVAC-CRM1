...
// Also convert LIKE to ILIKE for case-insensitive search
...
// Flatten spread args or single array into a plain array
...
// SQLite-compatible async db wrapper
const db = {
  prepare(sql) {
    return {
      async get(...args) {
        const { rows } = await pool.query(toPostgres(sql), flatten(args));
        return rows[0] || null;
      },
      async all(...args) {
        const { rows } = await pool.query(toPostgres(sql), flatten(args));
      ...
      async run(...args) {
        ...
        const result = await pool.query(finalSql, flatten(args));
        return {
  ...
  // Raw query helper (used by transaction internals and seed)
  async query(sql, params = []) {
    return pool.query(toPostgres(sql), params);
  },

  pool, // expose pool for direct client access when needed
};
...
const SCHEMA = `
  CREATE TABLE IF NOT EXISTS companies (
    ...
    annual_revenue DOUBLE PRECISION, notes TEXT,
  ...
  CREATE TABLE IF NOT EXISTS proposals (
    ...
    subtotal DOUBLE PRECISION DEFAULT 0, tax_rate DOUBLE PRECISION DEFAULT 0,
    tax_amount DOUBLE PRECISION DEFAULT 0, total_amount DOUBLE PRECISION DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS proposal_line_items (
    ...
    description TEXT NOT NULL, quantity DOUBLE PRECISION DEFAULT 1,
    unit TEXT DEFAULT 'ea', unit_price DOUBLE PRECISION DEFAULT 0,
    total_price DOUBLE PRECISION DEFAULT 0, sort_order INTEGER DEFAULT 0
  );
  ...
  CREATE TABLE IF NOT EXISTS deals (
    ...
    value DOUBLE PRECISION DEFAULT 0, probability INTEGER DEFAULT 20,
  ...
  CREATE TABLE IF NOT EXISTS jobs (
    ...
    duration_hours DOUBLE PRECISION DEFAULT 2, notes TEXT,
...
async function seedDemoData(force = false) {
  const existing = await db.prepare('SELECT id FROM companies LIMIT 1').get();
  if (existing && !force) return;

  if (force) {
    for (const t of ['tasks','activities','deals','jobs','proposal_line_items','proposals','contacts','companies']) {
      try { await pool.query(`DELETE FROM ${t}`); } catch (_) {}
    }
    try { await pool.query(`DELETE FROM users WHERE role = 'demo'`); } catch (_) {}
  }
  ...
  for (const c of companies) {
    const r = await db.prepare(
      `INSERT INTO companies (name,address,city,state,zip,phone,website,property_type,contract_type,num_hvac_units,annual_revenue,notes)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`
    ).run(c.name,c.address,c.city,c.state,c.zip,c.phone,c.website,c.property_type,c.contract_type,c.num_hvac_units||0,c.annual_revenue||0,c.notes);
    compIds.push(r.lastInsertRowid);
  }
  ...
  for (const c of contacts) {
    ...
    ).run(c.company_id,c.first_name,c.last_name,c.title,c.email,c.phone,c.is_primary);
    if (c.is_primary) contactIds[c.company_id] = r.lastInsertRowid;
  }

  // ── Helper ──
  const today = new Date();
  const dateStr = (daysAgo) => { const d = new Date(today); d.setDate(d.getDate() - daysAgo); return d.toISOString().split('T')[0]; };

  // ── Proposals ──
  const proposals = [
    { company_id:apexId,   contact_id:contactIds[apexId],   title:'Annual HVAC Maintenance Contract 2025',           proposal_number:'PRO-2025-001', status:'accepted', service_type:'Maintenance',  subtotal:23400,  tax_rate:6, tax_amount:1404,    total_amount:24804,  valid_days:30, notes:'Full-year preventive maintenance for 24 HVAC units.',                                                   terms:'Net 30. Service visits scheduled quarterly. Emergency response within 4 hours.', signed_by:'David Chen',    signed_at:dateStr(45)+'T14:22:00Z', created_at:dateStr(60) },
    { company_id:summitId, contact_id:contactIds[summitId], title:'Emergency Chiller Replacement — Unit 3',          proposal_number:'PRO-2025-002', status:'sent',     service_type:'Installation', subtotal:63679,  tax_rate:6, tax_amount:3820.74, total_amount:67500,  valid_days:14, notes:'Unit 3 chiller failure. Replacement with Carrier 30XA-200 ton unit.',               terms:'Net 15. 50% deposit required to order equipment. Estimated lead time 3 weeks.',   signed_by:null, signed_at:null, created_at:dateStr(12) },
    { company_id:metroId,  contact_id:contactIds[metroId],  title:'Commercial Kitchen Exhaust Cleaning (4 Locations)',proposal_number:'PRO-2025-003', status:'draft',    service_type:'Cleaning',     subtotal:7736,   tax_rate:6, tax_amount:464.16,  total_amount:8200,   valid_days:30, notes:'Annual hood and duct cleaning for all 4 Metro Restaurant locations.',                terms:'Net 30. Scheduling coordinated to minimize kitchen downtime.',                    signed_by:null, signed_at:null, created_at:dateStr(5)  },
    { company_id:ridgeId,  contact_id:contactIds[ridgeId],  title:'Preventive Maintenance Agreement — Phase 2 Buildings',proposal_number:'PRO-2025-004', status:'accepted', service_type:'Maintenance',  subtotal:17358,  tax_rate:6, tax_amount:1041.48, total_amount:18400,  valid_days:30, notes:'PM contract for 3 new buildings in Phase 2 expansion.',                             terms:'Annual contract. Quarterly visits. Parts billed separately.',                    signed_by:'Mark Thompson', signed_at:dateStr(30)+'T10:05:00Z', created_at:dateStr(50) },
    { company_id:summitId, contact_id:contactIds[summitId], title:'Boiler Room Upgrade & Controls Modernization',    proposal_number:'PRO-2025-005', status:'sent',     service_type:'Installation', subtotal:134000, tax_rate:6, tax_amount:8040,    total_amount:142040, valid_days:30, notes:'Full boiler room overhaul with new Honeywell building automation controls.',         terms:'Net 30. 30% deposit required. Project duration: 6 weeks.',                       signed_by:null, signed_at:null, created_at:dateStr(20) },
    { company_id:greenId,  contact_id:contactIds[greenId],  title:'District-Wide Summer HVAC Tune-Up Program',      proposal_number:'PRO-2025-006', status:'draft',    service_type:'Maintenance',  subtotal:29811,  tax_rate:6, tax_amount:1788.66, total_amount:31600,  valid_days:45, notes:'Comprehensive pre-season tune-up for all 5 school campuses before fall semester.', terms:'Net 45. Work to be completed during June–July school break.',                    signed_by:null, signed_at:null, created_at:dateStr(3)  },
    { company_id:apexId,   contact_id:contactIds[apexId],   title:'Refrigerant Leak Detection & Repair — Floors 18–24',proposal_number:'PRO-2025-007', status:'accepted', service_type:'Repair',       subtotal:3868,   tax_rate:6, tax_amount:232.08,  total_amount:4100,   valid_days:7,  notes:'Detected refrigerant loss during quarterly PM. Scope: leak detection, repair, and recharge.', terms:'Net 15. Work performed after hours to minimize tenant disruption.', signed_by:'Sarah Kim', signed_at:dateStr(15)+'T16:40:00Z', created_at:dateStr(18) },
    { company_id:ridgeId,  contact_id:contactIds[ridgeId],  title:'New Construction HVAC Design-Build — Building 7', proposal_number:'PRO-2025-008', status:'sent',     service_type:'Installation', subtotal:84434,  tax_rate:6, tax_amount:5066.04, total_amount:89500,  valid_days:30, notes:'Design-build contract for the new 22,000 sq ft Building 7 in Phase 3.',            terms:'Net 30. Progress billing: 25% mobilization, 50% rough-in, 25% completion.',     signed_by:null, signed_at:null, created_at:dateStr(8)  },
  ];
  ...
  for (const p of proposals) {
    const r = await db.prepare(
      `INSERT INTO proposals (company_id,contact_id,title,proposal_number,status,service_type,
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).run(p.company_id,p.contact_id,p.title,p.proposal_number,p.status,p.service_type,
    ...
    proposalIds[p.proposal_number] = r.lastInsertRowid;

    const lineItems = lineItemsMap[p.proposal_number] || [];
    for (const li of lineItems) {
      await db.prepare(
        `INSERT INTO proposal_line_items (proposal_id,description,quantity,unit,unit_price,total_price,sort_order)
         VALUES (?,?,?,?,?,?,?)`
      ).run(r.lastInsertRowid,li.description,li.quantity,li.unit,li.unit_price,li.total_price,li.sort_order);
    }
  }

  // ── Jobs ──
  const jobs = [
    { company_id:apexId,   contact_id:contactIds[apexId],   title:'Q1 Preventive Maintenance — Floors 1-16',         job_type:'maintenance', technician:'Marcus Johnson', status:'completed',  scheduled_date:dateStr(30), scheduled_time:'08:00', duration_hours:8, notes:'All 12 units on floors 1-16 serviced. Filters replaced. Coils cleaned.' },
    { company_id:summitId, contact_id:contactIds[summitId], title:'Emergency A/C Repair — ICU Wing',                  job_type:'repair',      technician:'Tyler Brooks',   status:'completed',  scheduled_date:dateStr(22), scheduled_time:'14:00', duration_hours:4, notes:'Compressor failure on RTU-6. Replaced capacitor and contractor. Unit restored.' },
    { company_id:ridgeId,  contact_id:contactIds[ridgeId],  title:'Filter Change & Inspection — Buildings 1-3',       job_type:'maintenance', technician:'Marcus Johnson', status:'scheduled',  scheduled_date:dateStr(-7), scheduled_time:'07:30', duration_hours:6, notes:'Monthly filter replacement and belt inspection.' },
    { company_id:metroId,  contact_id:contactIds[metroId],  title:'Kitchen Exhaust Cleaning — Broadway Location',     job_type:'cleaning',    technician:'Sandra Lee',     status:'scheduled',  scheduled_date:dateStr(-14), scheduled_time:'22:00', duration_hours:3, notes:'After-hours cleaning per fire code requirements.' },
    { company_id:summitId, contact_id:contactIds[summitId], title:'Pre-Season Boiler Inspection — Mechanical Room B', job_type:'inspection',  technician:'Tyler Brooks',   status:'in-progress',scheduled_date:dateStr(-3), scheduled_time:'09:00', duration_hours:5, notes:'Annual boiler inspection before heating season.' },
    { company_id:greenId,  contact_id:contactIds[greenId],  title:'Annual HVAC Inspection — Greenfield Middle School',job_type:'inspection',  technician:'Marcus Johnson', status:'scheduled',  scheduled_date:dateStr(-21), scheduled_time:'08:00', duration_hours:10, notes:'Full campus inspection. 8 rooftop units + cafeteria system.' },
  ];
  for (const j of jobs) {
    await db.prepare(
      `INSERT INTO jobs (company_id,contact_id,title,job_type,technician,status,scheduled_date,scheduled_time,duration_hours,notes)
       VALUES (?,?,?,?,?,?,?,?,?,?)`
    ).run(j.company_id,j.contact_id,j.title,j.job_type,j.technician,j.status,j.scheduled_date,j.scheduled_time,j.duration_hours,j.notes);
  }

  // ── Deals ──
  const deals = [
    { company_id:apexId,   contact_id:contactIds[apexId],   proposal_id:proposalIds['PRO-2025-001'], title:'Apex Tower 2025 Contract Renewal',           stage:'negotiation',  value:24804,  probability:85, service_type:'Maintenance',  close_date:dateStr(-30), notes:'Renewal of existing annual contract. Client wants 5% pricing hold.' },
    { company_id:summitId, contact_id:contactIds[summitId], proposal_id:proposalIds['PRO-2025-002'], title:'Summit Health Chiller Replacement',           stage:'proposal',     value:67500,  probability:65, service_type:'Installation', close_date:dateStr(-21), notes:'High urgency — existing unit unreliable. Board approval needed for spend over $50k.' },
    { company_id:metroId,  contact_id:contactIds[metroId],  proposal_id:null,                        title:'Metro Restaurant — Multi-Location Service Contract', stage:'qualification', value:45000,  probability:30, service_type:'Maintenance',  close_date:dateStr(-60), notes:'Tony Ricci shopping 3 other vendors. Our price is competitive.' },
    { company_id:greenId,  contact_id:contactIds[greenId],  proposal_id:proposalIds['PRO-2025-006'], title:'Greenfield District HVAC Modernization',     stage:'proposal',     value:185000, probability:50, service_type:'Installation', close_date:dateStr(-45), notes:'RFP submitted. Budget approved by school board. Decision expected Q2.' },
    { company_id:ridgeId,  contact_id:contactIds[ridgeId],  proposal_id:proposalIds['PRO-2025-008'], title:'Ridgeline Phase 3 — Building 7 Design-Build', stage:'negotiation',  value:89500,  probability:80, service_type:'Installation', close_date:dateStr(-15), notes:'Mark Thompson wants to start Q2. Contract language being reviewed.' },
  ];
  for (const d of deals) {
    await db.prepare(
      `INSERT INTO deals (company_id,contact_id,proposal_id,title,stage,value,probability,service_type,close_date,notes)
       VALUES (?,?,?,?,?,?,?,?,?,?)`
    ).run(d.company_id,d.contact_id,d.proposal_id||null,d.title,d.stage,d.value,d.probability,d.service_type,d.close_date,d.notes);
  }

  // ── Activities ──
  const activities = [
    { company_id:apexId,   type:'call',    subject:'Q2 planning call with David Chen',    body:'Discussed scope for summer PM work. David confirmed budget approved for chiller inspection on floors 17-32.' },
    ...
    { company_id:metroId,  type:'call',    subject:'Initial consultation — Tony Ricci',   body:'Tony wants all-in pricing for kitchen exhaust cleaning across 4 locations. Pricing competitive.' },
    ...
    { company_id:apexId,   type:'note',    subject:'Leak repair completed — floors 18-24',body:'Marcus located two pinhole leaks in refrigerant line. Braised and pressure-tested. Recharged with 12 lbs R-410A.' },
  ];
  for (const a of activities) {
    await db.prepare(
      `INSERT INTO activities (company_id,type,subject,body) VALUES (?,?,?,?)`
    ).run(a.company_id,a.type,a.subject,a.body);
  }
  ...
  for (const t of tasks) {
    ...
    ).run(t.company_id,t.title,t.due_date,t.priority);
  }
